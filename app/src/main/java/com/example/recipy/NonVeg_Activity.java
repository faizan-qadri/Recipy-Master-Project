...
public class NonVeg_Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Toolbar toolbar = findViewById(R.id.nonvegToolbar);
        setSupportActionBar(toolbar);
        getSupportActionBar().setTitle("");
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);

        cat_list = findViewById(R.id.nonVegList);
        ...
        firestore.collection("NonVeg_Data").orderBy("index", Query.Direction.DESCENDING).addSnapshotListener(new EventListener<QuerySnapshot>() {
            @Override
            public void onEvent(@Nullable QuerySnapshot value, @Nullable FirebaseFirestoreException error) {
                if (error != null){
                    Toast.makeText(NonVeg_Activity.this, error.getMessage(), Toast.LENGTH_SHORT).show();
                }
